...
def create_image(options: Options) -> torch.Tensor:
    red = [[0.5021, 0.2843, 0.1935],
           [0.8017, 0.5914, 0.7038]]
    green = [[0.1138, 0.0684, 0.5483],
             [0.8733, 0.6004, 0.5983]]
    blue = [[0.9047, 0.6829, 0.3117],
            [0.6258, 0.2893, 0.9914]]

    imageValues = [red, green, blue]
...
def tensor_network():
    ...
    input_tensor: torch.Tensor = torch.FloatTensor([0.4, 0.8, 0.5, 0.3]).to(options.device)
    weights: torch.Tensor = torch.FloatTensor([0.1, -0.5, 0.9, -1]).to(options.device)
    ...
    weights = (weights - loss.item() * weights.grad.data)
    """END TODO"""
